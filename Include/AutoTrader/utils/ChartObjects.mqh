...
private:
  ...
  bool seti(ENUM_OBJECT_PROPERTY_INTEGER prop, long val) {
    return ObjectSetInteger(m_chart_id, m_name, prop, val);
  }
  bool setd(ENUM_OBJECT_PROPERTY_DOUBLE prop, double v) {
    return ObjectSetDouble(m_chart_id, m_name, prop, v);
  }
  bool sets(ENUM_OBJECT_PROPERTY_STRING prop, string v) {
    return ObjectSetString(m_chart_id, m_name, prop, v);
  }
  bool geti(ENUM_OBJECT_PROPERTY_INTEGER prop, long &val) {
    return ObjectGetInteger(m_chart_id, m_name, prop, 0, val);
  }
  bool getd(ENUM_OBJECT_PROPERTY_DOUBLE prop, double &v) {
    return ObjectGetDouble(m_chart_id, m_name, prop, 0, v);
  }
...
public:
  ...
  bool Create(const int obj_type, const string name_prefix, const datetime t1, const double p1,
              const datetime t2 = 0, const double p2 = 0.0, const long chart_id = 0,
              const int subwin = 0) {
    ...
    bool ok    = ObjectCreate(m_chart_id, m_name, (ENUM_OBJECT)m_type, m_subwin, t1, p1, t2, p2);
    if(!ok) return false;
  ...
  bool Rectangle(const string name_prefix, const datetime t1, const double p1, const datetime t2,
                 const double p2, const long chart_id = 0, const int subwin = 0) {
    ...
    // Note: OBJPROP_TRANSPARENCY không tồn tại trong MQL5

    return true;
  }

  bool HLine(const string name_prefix, const double price, const color c = clrGold,
             const long chart_id = 0, const int subwin = 0) {
  ...
  bool VLine(const string name_prefix, const datetime t, const color c = clrSilver,
             const long chart_id = 0, const int subwin = 0) {
  ...
  bool Trend(const string name_prefix, const datetime t1, const double p1, const datetime t2,
             const double p2, const bool ray = false, const color c = clrDeepSkyBlue,
             const int width = 1, const long chart_id = 0, const int subwin = 0) {
  ...
  bool Arrow(const string name_prefix, const datetime t, const double price,
             const uchar arrow_code = 5, const color c = clrRed, // 5 = left price label
             const long chart_id = 0, const int subwin = 0) {
    if(!Create(OBJ_ARROW, name_prefix, t, price, 0, 0.0, chart_id, subwin)) return false;

    seti(OBJPROP_ARROWCODE, arrow_code);
    seti(OBJPROP_COLOR, c);
  ...
  bool SetZOrder(const long z) { return seti(OBJPROP_ZORDER, z); }

  bool SetText(const string txt, const int fsize = 10, const string font = "Arial",
               const color c = clrWhite) {
  ...
  bool UpdateRectangle(const datetime t1, const double p1, const datetime t2, const double p2) {
  ...
  bool TextAt(const string txt, const datetime t, const double p, const color c = clrWhite,
              const int fsize = 10, const string font = "Arial") {
    if(m_type != OBJ_TEXT) {
...
// Example usage với arrow codes đúng:
// void MarkEntry(const bool isSell, const datetime t, const double px){
//    ChartObject arr;
//    // Sử dụng arrow codes hợp lệ:
//    // 1 = upward arrow, 2 = downward arrow, 5 = left price, 6 = right price
//    arr.Arrow("ENTRY", t, px, isSell ? 2 : 1, isSell ? clrTomato : clrLime);
//    arr.SetZOrder(1);
// }
