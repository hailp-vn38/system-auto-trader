...
private:
  ...
  double m_minRangePts, m_maxRangePts, m_breakBufferPts;
  ...
  void UpsertRangeBox(const color col, const int width, const bool back, const bool fill) {
    if(m_cur.timeStart <= 0 || m_cur.timeEnd <= 0 || m_cur.priceHigh <= 0 || m_cur.priceLow <= 0)
      return;
    ...
    if(m_box.Name() == "" || !m_box.Exists()) {
      if(!m_box.Rectangle(boxName, m_cur.timeStart, m_cur.priceHigh, m_cur.timeEnd, m_cur.priceLow))
        return;
    } else {
      // Nếu tên hiện tại khác với tên chuẩn (qua ngày mới chẳng hạn) ➜ xoá và tạo lại để đúng name
      if(m_box.Name() != boxName) {
        ...
        m_box.UpdateRectangle(m_cur.timeStart, m_cur.priceHigh, m_cur.timeEnd, m_cur.priceLow);
      }
    }
...
public:
  ...
  RangeBreakout(IMarketData *md, int startHour, int startMin, int endHour, int endMin,
                double minRangePts = 0, double maxRangePts = 0, double breakBufferPts = 0,
      ...
      : m_md(md), m_startHour(startHour), m_startMin(startMin), m_endHour(endHour),
        ...
        m_breakBufferPts(breakBufferPts), m_lastDay(0), m_breakUsed(false), m_sym(""),
