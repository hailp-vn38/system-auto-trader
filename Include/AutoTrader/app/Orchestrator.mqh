...
private:
  ...
  bool HandleOrderCancellation() {
    if(m_orders == NULL || m_exitSig == NULL) return false;

    OrderInfo ord;
    if(!m_orders.FirstBySymbolMagic(m_sym, m_magic, ord)) return false;
    if(!m_exitSig.ShouldCancelOrders(m_sym, m_tf, m_magic)) return false;

    int deleted = m_exec.DeleteAllPendings(m_sym, m_magic);
    return deleted > 0;
  }
  ...
  bool HandleEntry() {
    ...
    for(int i = 0; i < ArraySize(signals) && i < MAX_ENTRIES_PER_TICK; i++) {
      ...
      Print("Signal[", i, "] received: type=", EnumToString(sig.type), " isSell=", sig.isSell,
            " price=", sig.price, " stopPoints=", sig.stopPoints, " sl=", sig.sl, " tp=", sig.tp);
      ...
      const double lots = (m_sizer != NULL) ? m_sizer.Lots(m_sym, sig.stopPoints, 0.10, m_magic)
                                            : 0.10;
      ...
      // Execute trade
      ulong ticket = 0;
      bool success = m_exec.OpenTrade(m_sym, m_magic, sig.type, lots, sig.price, sl_final, tp_final,
                                      m_deviation, ticket, sig.comment);
...
public:
  ...
  Orchestrator(const string symbol, const ENUM_TIMEFRAMES timeframe, const int deviation_points,
      ...
      : m_signal(signal), m_exec(exec), m_md(md), m_exitSig(NULL), m_trailer(NULL), m_risk(NULL),
        m_sizer(NULL), m_targets(NULL), m_orders(NULL), m_event(NULL), m_sym(symbol),
        m_tf(timeframe), m_deviation(deviation_points), m_magic(magic) {
  ...
  void SetOrders(IOrders *ords) { m_orders = ords; }
  void SetEventHandler(ITradeEventHandler *eh) { m_event = eh; }
